...
#' @param X covariates
#' @param beta1 regression coefficient
#' @param beta2 regression coefficient
...
#'
#' @examples mspe_PMS_Mis_FH_SUMCA(20,3,matrix(runif(60,0,1),nrow=20,byrow=TRUE)
#' ,c(1,1,1),c(1,1,1),10,2.5,10,10)
#'
mspe_PMS_Mis_FH_SUMCA=function(m,p,X,beta1,beta2,A,D,K,R)
{
...
for(r in 1:R){
  ...
  mu<-rep(0,m)
  for(i in 1:n){
    yib[i]<-as.numeric(X[i,]%*%beta1)+sqrt(A)*random1[i]+SD[i]*random2[i]
    mu[i]<-as.numeric(X[i,]%*%beta1)+sqrt(A)*random1[i]
  }
  for(i in (n+1):m){
    yib[i]<-as.numeric(X[i,]%*%beta2)+sqrt(A)*random1[i]+SD[i]*random2[i]
   ...
   ###############################EBLUP
    ...
    ###############################################
     ...
     for(k in 1:K){
       ...
       yib.k<-rep(0,m)
       for(i in 1:n){
         yib.k[i]<-as.numeric(X[i,]%*%BETA.hat)+SD[i]*random2[i]  +sqrt(A.hat)*random1[i]
       }
       for(i in (n+1):m){
         yib.k[i]<-as.numeric(X[i,]%*%BETA.hat)+SD[i]*random2[i]   +sqrt(A.hat)*random1[i]
       }
       ...
       ############################################
       for(i in 1:m){
         bias.MC[k,i]<-bias.MC[k,i]+D[i]*(A.hat/(A.hat+D[i])-A.hat.k/(A.hat.k+D[i]))-( mu.hat[i]-A.hat.k*yib.k[i]/(A.hat.k+D[i])-(D[i]*(t(X[i,])%*%BETA.hat.k))/(A.hat.k+D[i]))^2+
           (mu.hat[i]-A.hat*yib.k[i]/(A.hat+D[i])-(D[i]*(t(X[i,])%*%BETA.hat))/(A.hat+D[i]))^2
       }#loop m
     }#loop K
     ...
     for(i in 1:m){
       mspe.new[r,i]<-(A.hat*D[i])/(A.hat+D[i])+(mu.hat[i]-A.hat*yib[i]/(A.hat+D[i])-(D[i]*(t(X[i,])%*%BETA.hat))/(A.hat+D[i]))^2+bias.co[i]
     }
...
mspe.new<-apply(mspe.new,2,mean)

Par<-c(BETA.HAT, A.HAT)
MSPE.TRUE.Final<-c(MSPE.TRUE)
mspe.Sumca.Final<-c(mspe.new)
